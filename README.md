...
|---|---|
| 🏗 **App Builder** | Ask for an app name → logo (optional) → **technology** → a free-text description. Two targets: **📱 Flutter** → an installable **APK**, or **🌐 Web** (HTML/CSS/JS) → a **live website** hosted on GitHub Pages. A **design model** first produces a UI spec, then a **coder model** writes the app following it. Produces a full multi-screen **MVP**; when the description resembles a well-known app category (social feed, short-video, editor, chat, shop), it scaffolds that category's standard screens — as a **generic** app under the user's own name, never reproducing a real brand's name, logo, or trade dress. |
...
| `SPONSOR_ENABLED` | — | Master switch for the sponsor gate (default on); set `false` to disable `REQUIRED_CHANNELS` |
